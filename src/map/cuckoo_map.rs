...
use crate::{
    ...
    types::{Fingerprint, Key, OperationType, SlotStateFlags, Value}, 
...
pub struct CuckooMap<K: Key + Clone + Send + Sync, V: Value + Clone + Send + Sync> {
    ...
    // 迁移状态
    migration_state: RwLock<MigrationState<K,V>>,
...
impl<K: Key + Clone + Send + Sync, V: Value + Clone + Send + Sync> CuckooMap<K, V> {
    ...
    pub fn new(
        ...
        Self {
            ...
            migration_state: RwLock::new(MigrationState::NotStarted),
            ...
            // 迁移完成标志
            migration_completed: AtomicBool::new(true),
    ...
    pub fn from_components(
        ...
        let buckets = (0..config.initial_capacity)
            .map(|_| Arc::new(Bucket::new(version_tracker.clone())))
            .collect();
    ...
    pub fn clone(&self) -> Self {
        ...
        Self {
            ...
            migration_state: RwLock::new(migration_state_value),
            ...
            size: AtomicUsize::new(self.size.load(Ordering::Acquire)),
            migration_progress:  Arc::clone(&self.migration_progress),
            migration_completed: AtomicBool::new(self.migration_completed.load(Ordering::Acquire)),
    ...
    pub fn insert(&self, key: K, value: V) -> Result<(), CuckooError> {
        ...
        log_debug!("worker {:?} insert start: ", thread_id);
        ...
        loop {
            let result = {
                ...
                match migration_state {
                    ...
                    let result= self._insert(current_key.clone(), current_value.clone(), &version_guard);
                    if result.is_ok(){
                    ...
                    MigrationState::Migrating(ref new_buckets) => {
                       ...
                       // self.insert_to_new_table(&current_key, &current_value, new_buckets, &version_guard)
    ...
    let (mut h1, mut h2) = hasher.locate_buckets_with_capacity(key,new_bucket_count);
    ...
    loop {
        ...
        if let Some(bucket) = buckets.get(h1) {
            if let Some(slot_idx) = bucket.find_empty_slot() {
                return self.commit_insert_in_bucket(
                    bucket, slot_idx, &current_key, &current_value, current_fp, version_guard
                );
            }
        }
      ...
      //  version_guard.check()?;
        ...
        // 计算被踢出元素的新位置
         let hasher = self.hasher.read().unwrap();
        (h1, h2) = hasher.locate_buckets_with_capacity(&current_key,new_bucket_count);
        depth += 1;
...
fn commit_insert_in_bucket(
    ...
    if let Some(slot_idx) = self.try_direct_insert(h1, &key, &value, fp) {
        ...
        match self.commit_insert(h1, key.clone(), value.clone(), slot_idx, version_guard) {
            Ok(()) => {
    ...
    if let Some(slot_idx) = self.try_direct_insert(h2, &key, &value, fp) {
        ...
        match self.commit_insert(h2, key.clone(), value.clone(), slot_idx, version_guard) {
            Ok(()) => {
        ...
        log_debug!("worker {:?} no available slot in bucket2", thread_id);
    }

    // 3. 两个桶都无法直接插入，进入踢出逻辑
    log_debug!("worker {:?} starting cuckoo kick", thread_id);
...
/// 查找部分匹配的槽位（基于指纹）
    ...
    fn commit_insert(
        ...
        loop {
            let buckets = self.current.read().unwrap();
            let bucket = buckets.get(bucket_idx).ok_or(CuckooError::InvalidBucket)?;
            let slot = &bucket.slots()[slot_idx];
            ...
            match slot.try_lock(current_fp, version_guard) {
                Ok(guard) => {
                    ...
                    // 提交更改
                     let hasher = self.hasher.read().unwrap();
                    guard.commit(hasher.fingerprint(&key));
    ...
    for depth in 0..self.config.max_kick_depth {
        ...
        // 2. 取出旧数据
        let buckets = self.current.read().unwrap();
        let bucket = buckets.get(current_bucket_idx).ok_or(CuckooError::InvalidBucket)?;
        let slot = &bucket.slots()[slot_idx];
        ...
        current_value = V::from_bytes(&old_value_bytes).ok_or(CuckooError::ValueDeserialization)?;
         let hasher = self.hasher.read().unwrap();
        current_fp = hasher.fingerprint(&current_key);
        
        // 5. 计算旧数据的新位置
        let (new_h1, new_h2) = hasher.locate_buckets(&current_key);
        ...
        log_debug!("worker {:?} cuckoo_kick start: key={:?}, current_bucket_idx={}", thread_id, String::from_utf8_lossy(current_key.as_bytes()), current_bucket_idx);
        // 7. 检查循环 - 只比较桶索引和指纹
        ...
        if path.iter().any(|(bucket_idx, _, path_hash)| {
            *bucket_idx == current_bucket_idx && *path_hash == current_key_hash
            ...
            log_warn!("Worker {:?} Cuckoo kick cycle detected at depth {}", thread_id, depth);
                ...
                if cycle_retry_count < MAX_CYCLE_RETRIES {
                 ...
                 // 多次尝试后仍然循环，触发扩容
                log_warn!("Worker {:?} Failed to break cycle after {} attempts, triggering resize,load_factor {} ", 
                    thread_id, MAX_CYCLE_RETRIES,self.load_factor());
                // 触发扩容
                 let expand_factor = self.calculate_expand_factor();
                if let Err(e) = self.migrate(MigrationPlan::expand(expand_factor)) {
                    log_error!("Failed to start migration: {:?}", e);
                    return Err(CuckooError::MigrationFailure);
                }
                log_warn!("Worker {:?} migrate thread start , retry insert ,depth {}", thread_id, depth);
                // 重试插入
          ...
          //  log_warn!("Worker {:?} start try_direct_insert depth :{}", thread_id, depth);
            ...
            if let Some(slot_idx) = self.try_direct_insert(current_bucket_idx, &current_key, &current_value, current_fp) {
                log_warn!("Worker {:?} try_direct_insert depth :{}", thread_id, depth);
                return self.commit_insert(current_bucket_idx, current_key, current_value, slot_idx, version_guard);
            }
             log_warn!("Worker {:?} failed to  try_direct_insert depth :{}", thread_id, depth);
        }
        
        log_warn!("cuckoo_kick max depth reached: key={:?}", 
            String::from_utf8_lossy(current_key.as_bytes()));
        // 踢出路径超过最大深度，触发扩容
        let expand_factor = self.calculate_expand_factor();
        self.migrate(MigrationPlan::expand(expand_factor))?;
...
fn calculate_expand_factor(&self) -> f32 {
    ...
    // 获取队列中的待插入项数量
    let queue_size = self.queue_size() as f32;
    ...
    pub fn migrate(&self, plan: MigrationPlan) -> Result<(), CuckooError> {
        ...
        let thread_id = thread::current().id();
        log_info!("worker {:?} Migrating from {} start", thread_id, bucket_count);
        self.migration_completed.store(false, Ordering::SeqCst);
        // 计算新容量
        ...
        log_info!("worker {:?} Migrating from {} to {} buckets", thread_id,
        ...
        log_info!("worker {:?} change state to Migrating，start_migration_thread", thread_id);
        
        // 启动后台迁移线程
        self.start_migration_thread(new_buckets, bucket_count);
    ...
    fn start_migration_thread(
        ...
        thread::spawn(move || {
            let thread_id = thread::current().id();
            log_info!("worker {:?} Migration worker started", thread_id);
            
             // 获取总桶数
            let total_buckets =bucket_count;
            // 使用传入的桶数量
            let new_bucket_count = {
                let buckets = new_buckets.read().unwrap();
                buckets.len()
            ...
            for bucket_idx in 0..bucket_count {
                log_debug!("worker {:?} Migrating bucket {}", thread_id, bucket_idx);
                
                if let Err(e) = this.migrate_bucket(bucket_idx, &new_buckets,new_bucket_count) {
                    log_error!("worker {:?} Failed to migrate bucket {}: {:?}", thread_id, bucket_idx, e);
                }
                log_debug!("worker {:?} Migrating bucket {} finish", thread_id, bucket_idx);
                
                // 更新进度
                this.migration_progress.fetch_add(1, Ordering::Release);
                // 记录当前进度
               // let progress = this.migration_progress();
               // log_debug!("worker {:?} Migration progress: {:.1}%", thread_id, progress * 100.0);
            }
            
            // 迁移完成
            log_info!("worker {:?} Migration completed", thread_id);
            {
                let mut state = this.migration_state.write().unwrap();
                *state = MigrationState::Completed(new_buckets.clone());
            }
            log_info!("worker {:?} change state to Migration completed", thread_id);
            
            // 验证数据完整性
             this.validate_migration(&new_buckets);
            log_info!("worker {:?} change state to Migration validate_migration finish", thread_id);
            // 尝试完成迁移切换
            if let Err(e) = this.complete_migration() {
                log_error!("worker {:?} Failed to complete migration: {:?}", thread_id, e);
            }
          //   log_info!("worker {:?} change state to Migration Switched", thread_id);
            // 重置进度
          //  this.migration_progress.store(total_buckets, Ordering::Release);
        });
    }
    ...
    fn migrate_bucket(
        ...
        for slot in bucket_guard.slots() {
            ...
            // 插入到新表
           if let Err(e) = self.insert_to_new_table(&key, &value, new_buckets,new_bucket_count, &version_guard) {
            log_warn!("Failed to insert key to new table: {:?}, will retry later", e);
            self.insert_queue.lock().unwrap().push_back((key.clone(), value.clone()));
            // 将请求加入队列
        ...
        //         log_error!("Failed to insert key to new table after retry: {:?}", e);
        //     }
        // }
    ...
    /// 完成迁移
    pub fn complete_migration(&self) -> Result<(), CuckooError> {
        let thread_id = thread::current().id();
        
        // 获取新桶数组
        let new_buckets = match &*self.migration_state.read().unwrap() {
            MigrationState::Completed(new_buckets) => new_buckets.clone(),
            MigrationState::Migrating(new_buckets) => {
                // 如果还在迁移中，不能完成
                return Err(CuckooError::MigrationIncomplete);
            }
            _ => return Err(CuckooError::MigrationNotNeeded),
        };
        
        // 原子切换桶数组
        {
            let mut current_lock = self.current.write().unwrap();
            *current_lock = new_buckets.read().unwrap().clone();
        ...
        // 更新哈希策略
        {  
             let mut hasher_lock = self.hasher.write().unwrap();
        ...
        log_info!("worker {:?} Migration state set to Switched", thread_id);
         // 设置迁移完成标志
        self.migration_completed.store(true, Ordering::SeqCst);
    ...
    self.log_queue_status();
       //log_info!("worker {:?} process_insert_queue after migration {}", thread_id,self.is_migration_fully_completed());
...
pub fn is_migration_completed(&self) -> bool {
    ...
    while self.queue_size() > 0 && self.is_migration_fully_completed() {
        iteration += 1;
        log_info!("Processing iteration {}", iteration);
...
fn process_batch(&self, batch: Vec<(K, V)>, max_retries: usize, retry_delay_base: u64) {
    ...
    if !failed_items.is_empty() {
        ...
        self.insert_queue.lock().unwrap().extend(failed_items);
    }
}
...
fn try_insert_with_retry(
    ...
    pub fn log_queue_status(&self) {
        log_info!("Insert queue status: size={}, processing={},migration status={}",   self.queue_size(), self.is_queue_processing(),self.is_migration_fully_completed());
    }
    ...
    pub fn wait_for_completion(&self) {
        ...
        log_info!("wait_for_completion: size={}, processing={}, migration_state={}",
            queue_size, processing, migration_state);
...
mod tests {
    ...
    fn create_test_map() -> CuckooMap<ByteKey, Vec<u8>> {
        ...
        let hasher = Box::new(DoubleHashStrategy::new(config.initial_capacity,HashAlgorithm::XxHash));
